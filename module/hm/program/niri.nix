...
{
  # niri 配置由 Home Manager 管理，内容内联于此，不再引用 dotfile 中的 config.kdl
    ...
    text = ''
      ...
      */
      ...
      binds {
          ...
          Mod+Return hotkey-overlay-title="切换列宽预设" { switch-preset-column-width; }
          Mod+O hotkey-overlay-title="窗口预览 (Toggle Overview)" repeat=false { toggle-overview; }
          ...
          // -------- 系统操作 --------
          Mod+Shift+E hotkey-overlay-title="退出 Niri" { quit skip-confirmation=true; }  // 退出 Niri（跳过确认）
